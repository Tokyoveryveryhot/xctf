...
## level2
level0升级版，没有直接给出system("/bin/sh")函数，而是给出了system函数地址与/bin/sh字符串地址
...
system函数地址可以是plt表中的也可以是.got.plt表中的，直接在函数中的则不需要函数的返回地址，writeup的第五条的exp
...
## when_did_you_born
两个buffer间的偏移溢出，结果等于1926自动执行system("cat flag")语句，与hello_pwn题相同，不同点是脆弱点函数

payload:
```python
payload = 'a'*8+p64(1926)//v6在栈中是dd，因此用p64
```
脆弱点函数：
```c
gets(&v5)
...
4、写exp时字符前面加b，可转成bytes，避免报错。例如b'a'*(0x20-0x18)
...
lib.srand(1)	
for i in range(10) 
	sh.sendlineafter("numbers:",str(libc.rand()%6+1))
```
...
危险函数gets(&s)可以溢出，name为全局变量，地址不变，利用该空间存放/bin/sh。system函数地址由plt表给出
...
if ( pwnme == 8 ){system("cat flag");}
```

在 printf 中，使用 *<a_number_of_chars>%<number>$n*

 就可以将相应的第 *<number>* 个参数的位置写为 ：%前输出的字符数量。而pwnme是.bss段的全局变量，地址不变。
...
就可以将相应的第 *<number>* 个参数的位置写为 % 前输出的字符数量
